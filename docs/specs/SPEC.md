...
> this contract.
>
> **Amendments** (each ratified by the maintainer; the frozen baseline is
> 2026-07-06):
> - **Δ3 — ratified 2026-08-01.** Batch point-in-polygon moves from §3
>   (out of scope) to §2 (in scope), and **F7** / **F7b** join the §8 feature
>   list. Rationale: v1 shipped the single-address path; the deferred "later
>   feature" is now the next unit of work. §9's no-persistence rule is
>   unchanged and constrains it — see the §2 wording.
> - **Δ5 — ratified 2026-08-02.** A local layer-installation tool joins §2, and
>   **F8** joins the §8 feature list. Rationale: installing a polygon layer
>   required editing `scripts/build_data.py` and hand-writing TOML, which puts
>   the engine's layer-agnostic promise (§1) out of reach of the nonprofits and
>   small offices it was written for. §3 gains a matching boundary: the tool is
>   local-only, and remote layer administration stays out of scope because §3
>   still defers authentication.
...
- A **scaffolding bootstrap** that stands the project up from a parent projects
  directory (see §6).
- **A local layer-installation tool.** Adding a polygon layer must not require
  editing code. The tool accepts a candidate layer (a zipped or loose shapefile,
  GeoJSON, or an ArcGIS REST / export URL), validates it, **shows the operator
  the shape drawn over the layers already configured before anything is
  committed**, and then writes the layer into `config.toml`. Every rejection
  carries a stable error code and a plain-language fix. It runs on the
  operator's own machine, offline-capable, and is **never exposed by the
  deployed service**. *(Δ5, ratified 2026-08-02.)*
- **Batch point-in-polygon** over an address or coordinate list supplied as
  **CSV, XLSX, or a published Google Sheet** — processed **in memory and never
  persisted** (§9 binds this: the service retains nothing). Delivered first as a
  local CLI (**F7**), with an API endpoint to follow (**F7b**). Reading a
  link-shared Google Sheet via its public CSV export requires **no API key and
  no credentials**, consistent with §9's key-free rule. *(Δ3, ratified
  2026-08-01.)*
...
- ~~**Batch geocoding of large address lists.** v1 is single-address; batch may
  be a later feature.~~ *(Moved into §2 by Δ3, ratified 2026-08-01. Struck
  rather than deleted so the contract's history stays readable.)*
...
- **Remote layer administration.** The F8 tool (§2) is local-only: a separate
  application, bound to loopback, launched by hand, never imported by the
  deployed service. An authenticated admin surface on a running instance would
  mean accepting file uploads and config writes from the network, which this
  spec cannot sanction while authentication is deferred above. That is a
  separate feature requiring its own threat model. *(Δ5, ratified 2026-08-02.)*
...
- **F7** — Batch point-in-polygon from CSV / XLSX / published Google Sheet,
  delivered as a local CLI. Rows carry either an address to geocode or lat/lon
  already. In-memory only; nothing persisted (§9). *(Δ3, 2026-08-01.)*
  - **F7b** — The same engine behind an API endpoint; requires its own §4
    contract delta before it is built.
- **F8** — Local layer-installation tool: validate a candidate polygon layer,
  preview it over the configured layers, and write it into `config.toml` with a
  backup. Coded, plain-language errors. Local-only per §3. *(Δ5, 2026-08-02.)*
