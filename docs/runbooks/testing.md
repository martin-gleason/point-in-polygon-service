...
One of the runbooks in `docs/runbooks/` — operational procedure, not contract
(`specs/`) and not schedule (`plans/`). See `docs/README.md` for the full index.
...
smoke-test the offline path instead, follow `docs/runbooks/deployment.md` §2
to switch the default provider to `local_points` before running the curl block
above.
